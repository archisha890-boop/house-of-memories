...
export class IntroDriveScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.input.keyboard.on("keydown-E", () => this.openFocusedInteraction());
    this.input.keyboard.on("keydown-R", () => this.openLetter());
    this.input.keyboard.on("keydown-ESC", () => this.finishScene());
...
this.backgroundImage = bg;
    ...
    this.rainLayer = this.add.graphics().setDepth(32);
    this.fogBackLayer = this.add.graphics().setDepth(3);
    this.fogFrontLayer = this.add.graphics().setDepth(26);
    this.lightningLayer = this.add.graphics().setDepth(34);
    this.letterOverlay =  this.add.group();

    for (let i = 0; i < 115; i += 1) {
      this.rain.push({
        x: Phaser.Math.Between(-width * 0.2, width * 1.2),
        y: Phaser.Math.Between(-height, height),
        len: Phaser.Math.Between(8, 28),
        speed: Phaser.Math.Between(260, 590),
        slant: Phaser.Math.FloatBetween(-13, -4),
        alpha: Phaser.Math.FloatBetween(0.045, 0.16)
      });
    }
    ...
    for (let i = 0; i < 22; i += 1) {
      this.fog.push({
        ...
        y: Phaser.Math.Between(Math.floor(height * 0.34), Math.floor(height * 0.86)),
        w: Phaser.Math.Between(Math.floor(width * 0.18), Math.floor(width * 0.55)),
        h: Phaser.Math.Between(22, 82),
        speed: Phaser.Math.FloatBetween(4, 18),
        alpha: Phaser.Math.FloatBetween(0.018, 0.075),
        front: i > 10
      });
    }
  ...
  createInteractables() {
    ...
    this.windshieldHotspot = this.add.rectangle(width * 0.5, height * 0.31, width * 0.72, height * 0.36, 0xffffff, 0)
      .setDepth(45)
      .setInteractive({ useHandCursor: true })
      .on("pointerdown", () => this.inspectWindshield());

    this.mirrorHotspot = this.add.rectangle(width * 0.48, height * 0.15, width * 0.18, height * 0.08, 0xffffff, 0)
      .setDepth(46)
      .setInteractive({ useHandCursor: true })
      .on("pointerdown", () => this.inspectMirror());

    this.letterHotspot = this.add.rectangle(width * 0.69, height * 0.77, width * 0.22, height * 0.17, 0xffffff, 0)
      .setDepth(46)
      .setInteractive({ useHandCursor: true })
      .on("pointerover", () => this.setLetterGlow(true))
      .on("pointerout", () => this.setLetterGlow(false))
      .on("pointerdown", () => this.openLetter());

    this.hint = this.add.text(width * 0.05, height * 0.94, "E / Click - Interact     R - Reopen Letter     Esc - Skip", {
  ...
  update(_, deltaMs) {
    ...
    if (this.driveTime > 56 && this.phase < 1) {
      this.phase = 1;
  ...
  openLetter() {
    ...
    paper.fillRect(width * 0.28, height * 0.12, width * 0.44, height * 0.76);
    paper.lineStyle(3, 0x6b3f26, 0.9);
    paper.strokeRect(width * 0.28, height * 0.12, width * 0.44, height * 0.76);
    const text = this.add.text(width * 0.33, height * 0.18, LETTER_TEXT, {
      fontFamily: "IM Fell English SC, Georgia, Times New Roman, serif",
    ...
    const close = this.add.text(width * 0.5, height * 0.82, "Click to close", {
    ...
    this.letterOverlay.addMultiple([paper, text, close]);
    this.letterOverlay.getChildren().forEach((child) => child.setAlpha(0));
    this.tweens.add({ targets: this.letterOverlay.getChildren(), alpha: 1, duration: 260 });
    this.time.delayedCall(80, () => {
      this.input.once("pointerdown", () => this.closeLetter(), this);
    });
  }

  closeLetter() {
    if (!this.letterOverlay || this.letterOverlay.getChildren().length === 0) return;
